...
class ServerHelper:
    ...
    async def register(
        ...
        async with AsyncClient(timeout=Timeout(timeout=30)) as client:
            response = await client.post(
                f"http://0.0.0.0:7788/register",
                json=AgentInfo(name=name, desc=desc, type="Thing Assistant").model_dump(),
                timeout=60,
            )
    ...
    async def query_assistant(cls, name: list[str] | str):
        async with AsyncClient(timeout=Timeout(timeout=30)) as client:
            response = await client.post(
                f"http://0.0.0.0:7788/query_assistant",
                json={"name": name},
            )
            response.raise_for_status()

...
try:
    ...
    async_openai_client = openai.AsyncClient(
        ...
        http_client=httpx.AsyncClient(verify=False),  # ⚠️ not for production
    )
        ...
        **kwargs,
        ...
        try:
            ...
            response = await httpx.AsyncClient().post(
                "http://multi-purpose-ai-openai:5050/openai-chat",
                json={
                    "messages": messages,
            ...
            logger.error(f"[ConnectError] Failed to connect to OpenAI API: {e}")
            raise
        except httpx.ReadTimeout as e:
            logger.error(f"[ReadTimeout] Timed out while waiting for OpenAI API response: {e}")
            raise
        except httpx.HTTPStatusError as e:
            logger.error(f"[HTTPStatusError] HTTP error response from OpenAI API: {e.response.status_code} - {e.response.text}")
            raise
        except Exception as e:
            logger.error(f"[General Exception] Unexpected error in OpenAI call: {type(e).__name__} - {e}")
            raise
        ...
        #     if data_to_save["response"]["choices"][0]["message"]["content"] is not None:
        #         raw_content = data_to_save["response"]["choices"][0]["message"]["content"]
        #         try:
        #             raw_content = data_to_save["response"]["choices"][0]["message"]["content"]
        #             content = json.loads(raw_content)
        #         except json.JSONDecodeError:
        #             # Attempt to repair and parse again
        #             content = json.loads(JsonRepair(raw_content).repair())
        #     else:
        #         content = None
        # else:
        content = data_to_save["response"]["choices"][0]["message"]["content"]
            # if content is None:
            #     raise ValueError("The returned content is None. Retrying...")

        if data_to_save["response"]["choices"][0]["message"]["tool_calls"] is not None:
        ...
        return LLMResult(
            content=content,
            role="assistant",
            tool_calls=data_to_save["response"]["choices"][0]["message"].get("tool_calls"),
            send_tokens=data_to_save["response"]["usage"].get("prompt_tokens"),
            recv_tokens=data_to_save["response"]["usage"].get("completion_tokens"),
            total_tokens=data_to_save["response"]["usage"].get("total_tokens"),
        )
